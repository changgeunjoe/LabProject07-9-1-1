...
//	CGameObject *pGameObject = CGameObject::LoadGeometryFromFile(pd3dDevice, pd3dCommandList, pd3dGraphicsRootSignature, "Model/Apache");
	...
	//pApacheObject->SetPosition(XMFLOAT3(pTerrain->GetWidth() * 0.5f, fHeight + 1500.0f,
	//pTerrain->GetLength() * 0.5f));
	pApacheObject->SetScale(2.0f, 2.0f, 2.0f);
	pApacheObject->Rotate(0.0f, 15.0f, 0.0f);
	//플레이어의 위치가 변경될 때 지형의 정보에 따라 플레이어의 위치를 변경할 수 있도록 설정한다. 
	SetPlayerUpdatedContext(pTerrain);
//카메라의 위치가 변경될 때 지형의 정보에 따라 카메라의 위치를 변경할 수 있도록 설정한다.
	SetCameraUpdatedContext(pTerrain);
	SetChild(pApacheObject, true);
	OnInitialize();
